...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only include categoryId if it's selected
      ...
      // Link uploaded image to product if exists
      if (uploadedImageUrl && product.id) {
        try {
          await productService.addImage(product.id, uploadedImageUrl);
          alert('Product and image created successfully!');
        } catch (imgError) {
          console.error('Failed to add image to product:', imgError);
          alert('Product created but failed to add image. You can add it later from the product edit page.');
        }
      } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Images are automatically optimized and stored locally
            </p>
          </div>
        </div>
